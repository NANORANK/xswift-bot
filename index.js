...
// 🔧 สร้าง 3 embeds แบบรูปตัวอย่าง: แบนเนอร์บน + ข้อความบน (5 บรรทัด) + รูปขั้นเต็ม + ข้อความล่าง
function buildTicketPanelEmbeds(guild) {
  ...
  // 2) Embed ข้อความด้านบน (แบบรูปที่ 2 — 5 บรรทัดสวย ๆ)
  // ตัวอย่างตามที่ขอ: ใช้อักษรพิเศษและ emoji แบบในรูป
  const topLines = [
    "╭─ ✧˚₊‧  ticket rules  ‧₊˚✧ ─╮",
  ...
  const topEmbed = new EmbedBuilder()
    ...
    // ใส่ thumbnail เล็กมุมขวาบน (จะขึ้นบริเวณแถวหัว embed ตาม Discord)
    .setThumbnail(TICKET_SMALL_CORNER);

  // 3) Embed รูปขั้น (ขึ้นเต็ม width ตามที่ต้องการ) — รูปแสดงเป็นขั้น/แถบ (รูปที่ 3)
  const stepImageEmbed = new EmbedBuilder()
    .setColor(0xffb6dc)
    .setImage(TICKET_STEP_IMAGE);

  // 4) Embed ข้อความด้านล่าง (usage + สรุป + footer ว่าเปิดห้องได้เมื่อมีเรื่องจริง ๆ)
  const bottomLines = [
    "╭─────────────────╮",
    "✧˚₊‧  ticket usage  ‧₊˚✧",
    "╰─────────────────╯",
    ...
    "",
    "🧾 เปิดห้องได้เมื่อมีเรื่องจริง ๆ น้า • xSwift Hub Community"
  ];

  const bottomEmbed = new EmbedBuilder()
    .setColor(0xffb6dc)
    .setDescription(bottomLines.join("\n"))
    // footer / icon ให้เป็น divider เล็ก ๆ เผื่ออยากให้ดูเหมือนต้นฉบับ
    .setFooter({
      text: `เปิดห้องได้เมื่อมีเรื่องจริง ๆ น้า • ${guild.name}`,
      iconURL: TICKET_SMALL_CORNER
    });

  // ส่ง back เป็น array ของ embeds: banner, top text, step image, bottom text
  return [bannerEmbed, topEmbed, stepImageEmbed, bottomEmbed];
}
...
client.on("interactionCreate", async (i) => {
  ...
  if (i.isChatInputCommand()) {
    ...
    if (i.commandName === "ticketpanel") {
      ...
      // ส่งเป็น array ของ embeds (banner, text, step image, bottom)
